...
<head>
	...
	<script>
			...
			$("#dodo").change(function() {
				...
				$.ajax({
					...
					success: function(res) {
						...
						//res.city : city라는 키로 map을 찾음
						//res.city.length : city키에 있는 값들의 배열길이
						for(let i=0; i<res.city.length; i++){
							// res.city[i] : 키값만 찾아놓으면 배열처럼 사용가능
		...
		function fCheck() {
			...
			str += '<input type="button" value="다시검색" onclick="location.reload();" class="btn btn-warning btn-sm mt-2" />'
			$("#demo").html(str);
...
<p><br/></p>
<div class="container">
	<h2>Ajax를 활용한 값의 전달2(Map(HashMap<key,value>)처리)</h2>
	<hr/>
	<form name="myform">
		<h3>도시를 선택하세요</h3>
