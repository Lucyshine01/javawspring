...
<div class="container">
  	...
  	<tr>
  		<td>
				<c:if test="${sAMid eq 'admin'}">
					<a href="${ctp}/member/adminLogout.gu" class="btn btn-sm btn-danger">관리자 로그아웃</a>
				</c:if>
			</td>
  ...
  <table class="table table-borderless m-0 p-0">
  	<tr>
  		<td class="text-left">
  			...
  			<form method="post" action="${ctp}/guest/guestList?pag=${pag}">
  		...
  		<td class="text-right">
  			...
  			<c:if test="${pag>totPage}"><script>location.href="${ctp}/guest/guestList.gu?pag=${totPage}&pageSize=${pageSize}";</script></c:if>
  			
  			<!-- 첫페이지 / 이전페이지 / (현페이지번호/총페이지수) / 다음페이지 / 마지막페이지 -->
  			<c:if test="${pag > 1}">
  				<a href="${ctp}/guest/guestList?pag=1&pageSize=${pageSize}">[첫페이지]</a>
  				<a href="${ctp}/guest/guestList?pag=${pag-1}&pageSize=${pageSize}">[이전페이지]</a>
  			</c:if>
  			${pag}/${totPage}
  			<c:if test="${pag < totPage}">
  				<a href="${ctp}/guest/guestList?pag=${pag+1}&pageSize=${pageSize}">[다음페이지]</a>
  				<a href="${ctp}/guest/guestList?pag=${totPage}&pageSize=${pageSize}">[마지막페이지]</a>
  			</c:if>
  		</td>
  	</tr>
  </table>
  ...
  <div class="text-center">
		<c:forEach begin="1" end="${totPage}" varStatus="st">
			<c:if test="${pag eq st.count}">
				<a href="${ctp}/guest/guestList?pag=${st.count}&pageSize=${pageSize}" class="btn btn-warning" style="width: 40px">${st.count}</a>
			</c:if>
			<c:if test="${pag ne st.count}">
				<a href="${ctp}/guest/guestList?pag=${st.count}&pageSize=${pageSize}" class="btn btn-primary" style="width: 40px">${st.count}</a>
			</c:if>
		</c:forEach>
	</div>
  ...
  <c:forEach var="vo" items="${vos}" varStatus="st">
	  <table class="table table-bordered mb-0">
	  	<tr>
	  		<td colspan="2">방문번호 : ${curScrStartNo}
				<c:if test="${sAMid eq 'admin'}">
					<a href="javascript:delCheck(${vo.idx})" class="btn btn-sm btn-danger">삭제</a>
				</c:if>
				</td>
	  		<td colspan="2" style="text-align: right;">방문IP : ${vo.hostIp}</td>
	  	</tr>
	  	<tr>
