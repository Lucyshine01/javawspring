	...
	// 마커를 클릭했을 때 해당 장소의 상세정보를 보여줄 커스텀오버레이입니다
	  ...
	  function displayPlaces(places) {
	      ...
	      for ( var i=0; i<places.length; i++ ) {
	    	  ...
	    	  // 중심으로부터 1000m 반경 마커 제한	
          var linePath = [	// 중심과 장소의 좌표지정
          	new kakao.maps.LatLng(latitude, longitude),
          	new kakao.maps.LatLng(places[i].y, places[i].x)
          ];
          var poly = new kakao.maps.Polyline({
        	  path: linePath	// 지정된 좌표로 폴리선 생성
          })
          console.log(places[i].place_name + "와 마커와의 거리(m) : " + poly.getLength());
          if(poly.getLength() > 1000) continue;
	  ...
	  function changMap() {
		  marker.setMap(null);
			...
			latitude = document.getElementById(address).value.split("/")[0];
		  longitude = document.getElementById(address).value.split("/")[1];
		  
		  // 해당 좌표로 포지션지정
		  position  = new kakao.maps.LatLng(latitude, longitude);
		  map.setCenter(position); // 포지션으로 지도 중앙이동
