...
public class HomeController {
	...
	// 에디터에선 업로드후 바로 화면에 이미지를 출력시키기에 response 객체가 필요하다
	...
	public void imageUploadGet(MultipartFile upload,
		...
		// ckeditor에서 올린(전송) 파일을, 서버 파일시슽메에 실제로 저장할 경로를 결정한다.
...
//		FileOutputStream fos = new FileOutputStream(realpath + oFileName);
		...
		// 서블릿에서 매핑한 경로 /javawspring/board/**로 설정
		String fileUrl = request.getContextPath() + "/data/ckeditor/" + oFileName;
		// originalFilename : 예약어(파일명)
		// uploaded : 예약어(1:true)
